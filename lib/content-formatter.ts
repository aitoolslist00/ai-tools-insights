...
export function enhanceContentFormatting(html: string): string {
  ...
  // First, convert any remaining markdown bold (**text**) to HTML strong tags
  // This catches any markdown that wasn't processed by the marked library
  // Updated regex to handle various edge cases and punctuation
  // Matches ** followed by content (including special chars) followed by **
  result = result.replace(/\*\*([^*]+?)\*\*/g, '<strong>$1</strong>');
  
  // Additional pass to catch bold syntax with inline punctuation/apostrophes
  ...
  // Important: Add styling to strong/bold tags (these are generated by marked or our bold conversion)
  // We apply inline styles for guaranteed rendering
